...
inquirer.prompt(autocompleteOptions).then((result) => {
  const element = flatScripts.find(
    (element) => element.prettyString === result.string
  );
...
function getConfigPath() {
  ...
  try {
    if (existsSync(path.join(process.cwd(), "./.npsrc"))) {
      configPath = JSON.parse(
        readFileSync(path.join(process.cwd(), "./.npsrc"))
      ).config;
    } else if (existsSync(path.join(process.cwd(), "./.npsrc.json"))) {
      configPath = JSON.parse(
        readFileSync(path.join(process.cwd(), "./.npsrc.json"))
      ).config;
    }
  } catch (e) {
